...
-- check for missing geomShapeID

UPDATE local_authority."Bays_Transfer"
SET "GeomShapeID" = 1
WHERE "GeomShapeID" IS NULL;

UPDATE local_authority."Lines_Transfer"
SET "GeomShapeID" = 10
WHERE "GeomShapeID" IS NULL;
...
FROM local_authority."Bays_Transfer" l
...
SELECT DISTINCT l."RestrictionTypeID", 'LineString'
FROM local_authority."Lines_Transfer" l
WHERE l."RestrictionTypeID" IS NOT NULL
AND l."RestrictionTypeID" NOT IN (
...
ALTER TABLE IF EXISTS toms."Bays"
    ADD COLUMN IF NOT EXISTS "item_ref" integer;
ALTER TABLE IF EXISTS toms."Bays"
    ADD COLUMN IF NOT EXISTS "RBKC_NrBays" integer;

ALTER TABLE IF EXISTS toms."Bays" ALTER COLUMN IF EXISTS "CreateDateTime" SET DEFAULT now();
ALTER TABLE IF EXISTS toms."Bays" ALTER COLUMN IF EXISTS "LastUpdateDateTime" SET DEFAULT now();

ALTER TABLE IF EXISTS toms."Bays" ALTER COLUMN IF EXISTS "CreatePerson" SET DEFAULT current_user;
ALTER TABLE IF EXISTS toms."Bays" ALTER COLUMN IF EXISTS "LastUpdatePerson" SET DEFAULT current_user;

ALTER TABLE IF EXISTS toms."Bays" DROP COLUMN IF EXISTS "orig_GeometryID";

INSERT INTO toms."Bays"(
	geom, "RoadName", "RBKC_NrBays", "RestrictionID", "GeometryID", "RestrictionTypeID", "TimePeriodID",
	"GeomShapeID", "item_ref", "RestrictionLength")
SELECT (ST_Dump(geom)).geom AS geom,
    "Street_nam", "bNoBays", uuid_generate_v4(), "GeometryID", "RestrictionTypeID", "TimePeriodID",
    "GeomShapeID", "item_ref", ST_Length(geom)
	FROM local_authority."Bays_Transfer";
...
ALTER TABLE IF EXISTS toms."Lines" ALTER COLUMN "CreateDateTime" SET DEFAULT now();
ALTER TABLE IF EXISTS toms."Lines" ALTER COLUMN "LastUpdateDateTime" SET DEFAULT now();

ALTER TABLE toms."Lines" ALTER COLUMN "CreatePerson" SET DEFAULT current_user;
ALTER TABLE toms."Lines" ALTER COLUMN "LastUpdatePerson" SET DEFAULT current_user;

INSERT INTO toms."Lines"(
	geom, "RoadName", "RestrictionID", "GeometryID", "RestrictionTypeID", "NoWaitingTimeID",  "GeomShapeID", "item_ref", "RestrictionLength")
SELECT (ST_Dump(geom)).geom AS geom,
    "Street_nam", uuid_generate_v4(), "GeometryID", "RestrictionTypeID", "TimePeriodID",  "GeomShapeID", "item_ref", ST_Length(geom)
	FROM local_authority."Lines_Transfer";
...
/***
-- if using details from previous surveys ...
